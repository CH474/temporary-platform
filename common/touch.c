#include "common.h"

#include <stdio.h>
//#include <linux/input.h>
#include "input.h"
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <unistd.h>

static struct finger_info{
	int x;
	int y;
	int event;
} infos[FINGER_NUM_MAX];
static int cur_slot = 0;
//static int touch_fd;
int touch_init(char *dev)
{
	int fd = open(dev, O_RDONLY);
	if(fd < 0){
		printf("touch_init open %s error!errno = %d\n", dev, errno);
		return -1;
	}
	return fd;
}

/*return:
  TOUCH_NO_EVENT
  TOUCH_PRESS
  TOUCH_MOVE
  TOUCH_RELEASE
  TOUCH_ERROR
x: 0~1023
y: 0~599
finger: 0,1,2,3,4
*/
#define ADJUST_X(n) ((n*SCREEN_WIDTH)>>12)
#define ADJUST_Y(n) ((n*SCREEN_HEIGHT)>>12)
int touch_read(int touch_fd, int *x, int *y, int *finger)
{
	struct input_event data;
	int n, ret;
	if((n = read(touch_fd, &data, sizeof(data))) != sizeof(data)){
		printf("touch_read error %d, errno=%d\n", n, errno);
		return TOUCH_ERROR;
	}
//	printf("event read: type-code-value = %x-%x-%x\n", data.type, data.code, data.value);
	switch(data.type)
	{
		//default:		break;
	case EV_ABS:
//		printf("event read: type-code-value = %x-%x-%d\n", data.type, data.code, data.value);
		switch(data.code)
		{
		case ABS_MT_SLOT:
		//case 330:
			if(data.value >= 0 && data.value < FINGER_NUM_MAX){
				int old = cur_slot;
				cur_slot = data.value;
				if(infos[old].event != TOUCH_NO_EVENT){
					*x = infos[old].x;
					*y = infos[old].y;
					*finger = old;
					ret = infos[old].event;
					infos[old].event = TOUCH_NO_EVENT;
					return ret;
				}
			}
			break;
		case ABS_MT_TRACKING_ID:
			if(data.value == -1){
				*x = infos[cur_slot].x;
				*y = infos[cur_slot].y;
				*finger = cur_slot;
				infos[cur_slot].event = TOUCH_NO_EVENT;
				return TOUCH_RELEASE;
			}
			else{
				infos[cur_slot].event = TOUCH_PRESS;
			}
			break;
		case ABS_MT_POSITION_X:
			infos[cur_slot].x = ADJUST_X(data.value);
			if(infos[cur_slot].event != TOUCH_PRESS){
				*x = infos[cur_slot].x;
				*y = infos[cur_slot].y;
				*finger = cur_slot;
				infos[cur_slot].event = TOUCH_MOVE;
				return TOUCH_MOVE;
			}
			break;
		case ABS_MT_POSITION_Y:
			infos[cur_slot].y = ADJUST_Y(data.value);
			if(infos[cur_slot].event != TOUCH_PRESS){
				*x = infos[cur_slot].x;
				*y = infos[cur_slot].y;
				*finger = cur_slot;
				infos[cur_slot].event = TOUCH_MOVE;
				return TOUCH_MOVE;
			}
			break;
		}
		break;
	/*case 0x03:
		switch(data.code){
			case 0:
			for(cur_slot=0;cur_slot<5&&infos[cur_slot].x!=0;cur_slot++);
			if(cur_slot==5){
			cur_slot=0;
			break;}
			infos[cur_slot].x=ADJUST_X(data.value);
			break;
			case 1:
			//;
			infos[cur_slot].y=ADJUST_Y(data.value);
			break;
		}
		break;
	case 0:
		if(data.code==0){
			int i;
			for(i=0;i<=cur_slot;i++){
				x[i]=infos[i].x;
				y[i]=infos[i].y;
				infos[i].x=0;
				infos[i].y=0;
			}
			cur_slot=0;
		}*/
	case EV_SYN:
		switch(data.code)
		{
		case SYN_REPORT:
			if(infos[cur_slot].event != TOUCH_NO_EVENT){
				*x = infos[cur_slot].x;
				*y = infos[cur_slot].y;
				*finger = cur_slot;
				ret = infos[cur_slot].event;
				infos[cur_slot].event = TOUCH_NO_EVENT;
				return ret;
			}
			break;
		}
		break;
	}
	//return 1;
	return TOUCH_NO_EVENT;
}
